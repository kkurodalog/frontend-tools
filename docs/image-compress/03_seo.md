...
#### 逆導線CTAコピー案（★採用＝案A・黒田さん確定 2026-06-17 / Sara作成 2026-06-17 / 汎用化同期 2026-06-19）
...
>
> **🔄 2026-06-19 汎用化同期（黒田さん指示）**: 制作者カードは**共通 partial `maker-card.html`＝全ツールで使い回す**前提に変更。よって案A本文も**ツール特化文（冒頭「『画像仕分け圧縮くん』は、」）を廃し、ツール名に依存しない汎用文に置換**した（実装が確定済み）。理由＝今後ツールが増えても同じ partial を文言修正なしで再利用できるようにするため。本ドキュメント（正本）を実装 `maker-card.html` の文言に合わせて同期した。
>
...
**▼案A（作り手の顔を出す・実績先行型）★採用・汎用文（共通 partial = maker-card.html と一致 / 2026-06-19 同期）**
...
- ★リード文は**ツール名を含まない汎用文**（共通 partial `maker-card.html` で全ツール共通使用）。旧版はツール特化の書き出し（冒頭にツール名）だったが、2026-06-19 に上記汎用文へ同期更新。
...
- **検証済み: 逆導線CTA汎用化同期（2026-06-19）** — §6-2 案A本文をツール特化文（冒頭「『画像仕分け圧縮くん』は、」）から実装済みの汎用文へ一字一句同期（共通 partial `maker-card.html` と一致）。見出し「このツールを作った人について」・ボタン構成 `[制作実績を見る]`＋`[制作のご相談をする]`・案A採用方針は維持。黒田さん指示で全ツール共通 partial として使い回す前提に変更したのが理由。経緯・変更日を §6-2 内に追記。
