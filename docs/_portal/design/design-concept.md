...
作成日: 2026-06-16（Phase1）/ 2026-06-16 更新（Phase2 黒田さんレビュー反映）/ 2026-06-16 更新（Phase3 第2次レビュー反映）/ 2026-06-16 更新（Phase4 第3次レビュー反映）/ 2026-06-16 更新（Phase5 フッターナビ最終確定反映）/ 2026-06-16 更新（Phase6 黒田さん修正3点反映）/ 2026-06-18 更新（Phase8 ツールカードを本体 c-work-card に統一）/ 2026-06-19 更新（Phase9 カード hover を画像ズームのみに刷新・色変化/浮き/影/クリック廃止）
...
- ホバー: **サムネ画像（`<picture>` 内の `__thumb`）のみ `transform: scale(1.06)` でズーム**。`<picture>` ラッパーの `overflow: hidden` でズームをクロップし、`--radius-md` をラッパーに持たせて角丸を維持。`prefers-reduced-motion: reduce` でズーム無効。**カード自体の浮き（`translateY`）・影（`box-shadow`）は無し。クリック（`:active`）の視覚フィードバックも無し**（フラット基調で、動きは画像ズームのみに限定）。※最終確定は Phase9 §12（2026-06-19）。
- 全面リンクカードは hover で文字色を変えない（子テキスト色を base/muted で固定）。タイトル（`__name`）・タグの色も hover で変えない（Phase9 で accent 化を一度試したが黒田さん判断で不採用 → 削除）。
...
| カード | bg-elevated ＋ 1px border ＋ radius-md を踏襲（ツールカードに適用）。hover = サムネ画像のみ `scale(1.06)` ズーム（浮き・影・クリックフィードバック・色変化は無し / Phase9 §12 確定） |
...
| カード hover | border-color が accent に変わるだけ | 本体 work-card と同じ＝**サムネ画像のみ `scale(1.06)` ズーム**（浮き・影・クリックフィードバック・色変化は無し / Phase9 §12 確定）。リンク下線も消す |
...
- **カードグリッド**: `repeat(auto-fill, minmax(280px, 1fr))`、gap `--space-5`。各カードは本体 work-card 準拠（bg-elevated / 1px border / radius-md / hover = サムネ画像のみ `scale(1.06)` ズーム・全面リンク。浮き・影・クリックフィードバック・色変化は無し / Phase9 §12）。
- **カード内（Phase8 更新 / 2026-06-18 — 本体 work-card に統一）**: **サムネ画像（16:9 `<picture>`・webp+jpg フォールバック・`object-fit: cover`・`width/height` 属性で CLS 回避・`alt`・`loading="lazy"`）→ ツール名（`__name`・H2・`--fs-lg`・見出しフォント・base 色）→ タグライン（`__tagline`・`--fs-sm` muted）→ 下端タグ行（`__tags` > `c-badge`×N・`tools.json` の `tags` 配列を出力・`margin-block-start: auto` で下端揃え）**。構成順・font/余白/色は本体 `c-work-card` と同値。hover 挙動も本体 `c-work-card` と同値＝**サムネ画像のみ `scale(1.06)` ズーム**（浮き・影・クリックフィードバック・色変化は無し / Phase9 §12 で最終確定）。`status: published` のみ表示。将来「準備中」を出すなら別バッジ（warning 系）。
...
| 6 | カード hover/click を本体準拠 | ~~hover=`translateY(-2px)`＋`shadow-md`／active=`translateY(0)`＋`shadow-none`＋`transition-zero`~~ → **Phase9（§12 / 2026-06-19）で最終刷新**: hover = サムネ画像のみ `scale(1.06)` ズーム／浮き・影・クリックフィードバック・色変化は無し（本体 work-card と同値） | mock `.c-tool-card:hover/:active` → 実装 `_c-tool-card.scss` |
...
#### (f) カード hover/active（出典: `assets/styles/components/_c-work-card.scss` L26-42 / ※当時の本体実値・Phase9 で刷新済み）

> **⚠ この (f) は Phase3 時点（2026-06-16）の本体 work-card 実値であり、Phase9（§12 / 2026-06-19）で本体・Tools とも hover 仕様が「サムネ画像ズームのみ」に刷新されたため、以下の translateY+shadow+active 記述は履歴（旧本体実値）として残置。最終確定は §12 を参照。**

- 初期: **影なし**（フラット）。
- hover（旧）: `box-shadow: --shadow-md` ＋ `transform: translateY(-2px)`（top works プレビュー `.c-card--click` と同値）。
- active（旧・クリック中）: `box-shadow: --shadow-none !important` ＋ `transform: translateY(0)` ＋ `transition: transform --transition-zero`（押下を即反応に）。
- **→ Phase2 モックは hover のみで active 状態が無かった**。本体実値どおり active（戻り＋影消し＋transition-zero）を `.c-tool-card:active` に追加した（修正6）。**※本記述は Phase9 で全面刷新済み（§12）＝現行は浮き・影・active なし／hover は画像 `scale(1.06)` ズームのみ。**
...
- **font/余白/色**: 本体 `c-work-card` と同値。**hover 挙動はその後 Phase9（§12 / 2026-06-19）で「サムネ画像 `scale(1.06)` ズームのみ・浮き/影/クリック/色変化なし」に刷新**（本 Phase8 時点の §6-2(f) 旧記述は §12 で上書き）。`block-size:100%` は `<li>` グリッドアイテム内の `<a>` を行高いっぱいに伸ばし下端タグ揃えを効かせるため追加。
...
## 12. Phase9 黒田さん指示: カード hover を画像ズームのみに刷新（実装反映 / 2026-06-19）

カードの hover/click 挙動を試行錯誤の末に最終確定した。**これは概念モック段階ではなく F5-4 で本番実装（`dev/src/`）に反映済み**の確定変更（Cody 実装 / Reid 評価ゲート合格・本体 `c-work-card` 計14インスタンス＋ Tools `c-tool-card` で整合）。本節は design-concept 正本を実装の確定仕様に同期するための記録（旧 §1-5 / §2-1 / §2-3 / §3-1 / §6-1#6 / §6-2(f) / §11-2 の hover 記述を上書き）。

### 12-1. 最終確定した hover/click 仕様

| 項目 | 旧（Phase3〜8） | 新（Phase9 / 2026-06-19 確定） |
|---|---|---|
| hover | カード全体が `translateY(-2px)` ＋ `box-shadow: --shadow-md`（浮き＋影） | **サムネ画像（`__thumb`）のみ `transform: scale(1.06)` でズーム** |
| 画像クロップ | — | `<picture>` ラッパーの `overflow: hidden` でズームをクロップ・`--radius-md` をラッパーに持たせ角丸維持 |
| タイトル・タグの色 | （Phase9 で一度 accent 化を試行） | **hover で変えない**（`__name` は base 固定・タグ色も不変。accent 化は黒田さんが実物を見て不採用 → 削除） |
| カードの浮き・影 | あり（translateY＋shadow-md） | **無し** |
| クリック（`:active`）フィードバック | `translateY(0)`＋`shadow-none`＋`transition-zero` | **無し**（`:active` ルール自体を持たない） |
| a11y | — | `prefers-reduced-motion: reduce` でサムネズーム（transform）を無効化 |

### 12-2. 確定の経緯

1. 黒田さん指示でカード hover を刷新（旧 translateY＋shadow から脱却）。
2. 当初案では hover でサムネ画像ズーム ＋ タイトル/タグの色を accent 化していた。
3. 黒田さんが実物を見て**色変更（accent 化）を不採用** → タイトル・タグの色変化を削除。
4. 結果、**hover = サムネ画像 `scale(1.06)` ズームのみ／浮き・影・クリックフィードバック・色変化はすべて無し**に最終確定。

### 12-3. 出典

- Reid 合格レポート: `docs/_portal/_f5-4-reid-card-hover-review.md`（最終判定: 合格 / Must 指摘なし）
- 実コード（正本）: `dev/src/assets/styles/components/_c-tool-card.scss`（hover = `.c-tool-card__thumb { transform: scale(1.06) }`・`> picture { overflow:hidden; border-radius:--radius-md }`・`__name` color base 固定・`@media (prefers-reduced-motion: reduce)` でズーム無効・`:active` ルールなし）
- 本体側同値実装: kurodafolio `dev/src/assets/styles/components/_c-work-card.scss`

> ※Reid レポート観点1 には「hover で画像 scale(1.06) ＋ title/name 色 accent」と記されているが、これはレビュー時点の状態であり、その後の黒田さん判断で**色変更は削除**された（最終実装 SCSS が正本＝色変化なし）。

## 検証済み

- 検証済み（Phase9）: カード hover/click 記述を最終確定状態（hover = サムネ画像のみ `scale(1.06)` ズーム／浮き・影・クリックフィードバック・色変化なし／`prefers-reduced-motion` でズーム無効）に同期更新。旧「translateY+shadow / active 戻り」の能動記述を §1-5・§2-1・§2-3・§3-1（カードグリッド/カード内）から除去し、§6-1#6・§6-2(f) は取り消し線＋上書き注記（経緯保持）、§11-2 は §12 への参照に更新。タイトル/タグの色変化なし（accent 化は試行の末不採用）を §1-5・§12 で明確化。実装 SCSS（`_c-tool-card.scss` = hover で `__thumb` のみ scale・`__name` base 固定・`:active` ルールなし・reduced-motion で transform:none）および Reid 合格レポート（`_f5-4-reid-card-hover-review.md`）と一致。
- 検証済み（Phase9）: §12 を新設し、変更日（2026-06-19）・経緯（hover 刷新→色変更は黒田さん判断で不採用→画像ズームのみに確定）・新旧対応表・出典パスを記録（後から経緯が辿れる）。Reid レポート観点1 の「色 accent」記述が削除前の状態である旨も注記し、最終実装 SCSS を正本として明示。
- 検証済み（Phase9）: 周辺の体裁・記法・粒度（Phase 見出し＋対応表＋出典併記＋検証済みブロック）を踏襲し、hover/click と無関係な箇所（§5・§7〜§10・tokens・カラー/タイポ/余白記述）は不変更。本番 CSS（dev/src/）も書き換えていない（読み取りのみ）。
