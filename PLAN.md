...
### 2-10. 第2弾ツール: AI画像プロンプト・ビルダー（仮称 / U-04 決着 2026-06-19）

第2弾は候補カタログ（§2-7）からではなく、**黒田さん発案の独自ツール**に確定。候補スコアリング（Mado）はスキップ。
...
- **実装難所**: エンジニアリングは軽い（フォーム＋テンプレートエンジン＋コピー機能）。本質は**質問フロー設計＋AI臭さ消しのプロンプト構築知識ベース**＝Sara/Mado/黒田さんの craft 言語化が主役（Cody実装より上流が肝）。
- **F2で詰める未決**: ①ツール名・**slug**（確定後は不変＝301回避）②質問フロー設計＆"AI臭さ消し"知識ベースの中身 ③日英切替UIの具体 ④受け渡しUI＋参考AI注記の出し方。
...
| ~~U-04~~ | 第2弾ツールの決定 | **✅決定済み（2026-06-19）＝候補カタログ外の黒田さん発案ツール「AI画像プロンプト・ビルダー（仮称）」に確定**。決定論的ビルダー単体／日英切替／モデル非依存／受け渡し型（markloom型）。候補スコアリング（Mado）はスキップ。詳細＝§2-10。次＝F2設計 | ~~第1弾公開後~~ **決定済み** | 黒田さん（候補外発案） |
...
| ~~P-2~~ | 第2弾ツールの決定（U-04） | **✅決着（2026-06-19）＝AI画像プロンプト・ビルダー（仮称・候補カタログ外の黒田さん発案）に確定**。決定論的ビルダー単体／日英切替／モデル非依存／受け渡し型（markloom型）。次フェーズ＝**F2設計**（Mado=overview / Mane=実現性・工数 / Sara=SEO・コピー）。OGP・共通枠・デプロイ手順は第1弾で確立済み（再利用OGPテンプレ＝`docs/_portal/design/ogp-template/`）＝量産しやすい。詳細＝§2-10 | F2へ移行 |
