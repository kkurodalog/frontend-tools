...
| F5 本体側導線追加 | 🔄 ①③先行実装完了／②後続 | **設計方針＝Haru作成→Mado評価ゲート条件付き合格→Must3/Should4/Nice2反映済み**（正本`docs/_portal/03_main-site-tools-routing-design.md`／評価`_f5-mado-routing-review.md`）。**黒田さん★確定（2026-06-18）**＝①ナビ・③フッターだけ先行／②トップ「公開ツール」sectionは後続（サムネ画像=ogp.png連動が前提）／ラベル日本語「ツール」／②背景交互は案B完全交互（②実装時）／フッター単一行のまま。**①③＝Cody実装完了**（本体`kurodafolio/06_implementation/dev/` src 3ファイル各1行追加・`yarn build`green・dist出力＋`aria-current=false`裏取り・差分①③限定）→**Reid評価ゲート条件付き合格（Must0・差し戻しなし）**。**残=黒田さん実機目視1点**（footer xl帯`flex-wrap:nowrap`で7リンク収まり＋PCナビ5リンク・窮屈時のみ`_p-footer.scss`/nav gap微調整／CSS変更は現状不要判断）。**②着手の前提**＝サムネ画像準備＋★4リード文(Sara→黒田さん確定)。本体dev/はローカル未commit（F6デプロイ前） |
