...
ステータス: **F1 完了（プロジェクト初期化）／ Fd 完了（デザイン設計・Mado条件付き合格）／ 方向A 完了（Fd 本番CSS実装＝共通枠・Reid評価ゲート通過 → さらに static-template 基盤フルリビルドで再構築・Reid再合格）／ デザイン確定（ナビ案B・768px境界・カテゴリ非表示）／ i18n＝採用しない（2026-06-17確定）／ アーキ＝独立継続（コピー運用・2026-06-17確定）／ F2 第1弾機能設計 完了（2026-06-17・Mane評価ゲート条件付き合格→Must/Should反映済み）／ **F3 第1弾実装 完了（2026-06-18・Stage1〜4＝WebP/AVIF/PNG可逆/④減色/keep-JPEG・種別ラジオ＋複数フォーマット出力・自動推測・個別削除・SEO/JSON-LD/CTA案A・黒田さん実機修正多数）／ F4 最終評価ゲート通過（Reid 条件付き合格→Must M-2/S-1/S-3反映→後発UI変更の再チェックも合格）／ プロジェクト名を frontend-tools にリネーム（旧 kuroda-tools・2026-06-18・GitHub Public 公開）／ F5本体導線①②③＋逆導線 完了・OGP3枚 完了 ／ **F6 デプロイ・公開 完了（2026-06-19・`kurodafolio.com/tools/` 本番公開・黒田さん実機確認済み・kiso.css 404/MIME は公開後修正で解消・Mane公開チェック＋代表確認済み）＝第1弾「画像仕分け圧縮くん」クローズ。次＝公開後申し送り（記事⇄ツール文脈リンク回遊／第2弾優先順 U-04）＝§8-3 参照** ／ **ナビ整合改善（パンくずA案＋ラベル和文化＋ポータルパンくず追加＋「制作者について」導線＋フッターラベル統一）完了・本番公開・push済（2026-06-19）** ／ **U-04 決着（2026-06-19）＝第2弾＝AI画像プロンプト・ビルダー（slug=`ai-image-prompt`・候補カタログ外の黒田さん発案）に確定。決定論的ビルダー単体／日英切替／モデル非依存／受け渡し型** ／ **第2弾 F2（機能設計）完全クローズ（2026-07-01）＝光/質感プリセット実機確定（Google Flow/Imagen 4）・被写体タイプ分岐（人物=光/物・空間=質感）・spec本体完成（`01_spec.md`＝9カテゴリ質問フロー・二層構造・カテゴリ別v1天井≈60 optionKey・決定論ロジック・日英対辞書・ネガティブ別欄重み無し・人物肌質感は⑦person_poresに一本化）・spec評価ゲート条件付き合格＝F3着手可（Mane）・SEO/QA完成（`03_seo.md`/`02_qa.md`）。次＝F3実装（Cody・static-template基盤・工数S〜M＝実働3〜6日）。§2-10／§8 進捗表 参照**
...
### 2-7. 今後の候補（第3弾以降）
clamp メーカー（slug=`clamp`）／ 配色＆コントラスト（`color-palette`）／ FLOCSS スキャフォルダ（`flocss-generator`）等。出典: `docs/_portal/mado-selfuse-dev-tools.md` D-1/A-5/D-2、候補カタログ＝`docs/_portal/mado-candidates.md`。
**【U-04 決着（2026-06-19）】**: 第2弾は本カタログ外の黒田さん発案ツール「AI画像プロンプト・ビルダー（仮称）」に確定（候補スコアリングはスキップ）＝§2-10。本候補群（clamp / color-palette / flocss-generator / motion）は**第3弾以降の候補として温存**。
...
### 2-10. 第2弾ツール: AI画像プロンプト・ビルダー（slug=`ai-image-prompt` / U-04 決着 2026-06-19 / F2前半 確定 2026-06-19）

第2弾は候補カタログ（§2-7）からではなく、**黒田さん発案の独自ツール**に確定。候補スコアリング（Mado）はスキップ。**表示名＝「AI画像プロンプト・ビルダー」**（slug 確定に伴い「仮称」を外す）。

- **痛点**: Web制作で使う画像をAI生成すると「いかにもAIっぽい」絵になり、ユーザーの心を掴めない。AI臭さを抑えた自然な画像を作りたい。
- **解決アプローチ**: ツールは「どんな画像を作りたいか」を段階的な質問ガイドで詰め、AI臭さを抑える要素を組み込んだ**具体的プロンプトを生成**。画像生成自体はユーザーが各自のAIで行う（**受け渡し型＝markloomモデル**／ツールの仕事はプロンプト生成まで）。
- **確定方針（黒田さん承認 2026-06-19）**:
  1. **コア方式＝決定論的ビルダー単体**（AI API接続なし・ブラウザ完結・無料・キー不要）。「壁打ち＝AIとの対話」ではなく「**構造化質問ガイド＋AI臭さ消しの知識ベース**」として実装。BYOK／Claude Code Skill配布は不採用（広い層に届かない／検索流入の旨味を失う）。
  2. **出力言語＝日英両方（切替）**。画像AIは英語に強いものが多く、日本語ユーザーの可読性も両立。
  3. **対象AI＝モデル非依存（汎用プロンプト）**。受け渡し時に「画像生成に強いAIの例（**2026年時点**: Flux 2 / Imagen 4 / GPT Image 2 等）」を注記として添える（モデルは入れ替わるので年付き・過剰断定しない）。
  4. **差別化＝「Web制作で使う画像 × AI臭さを消す × フリーランスWeb屋の知見」**の用途特化。汎用プロンプト生成器との差をここで出す。
  5. **コピー方針＝過剰約束の禁止**。「AI臭さを必ず消す」ではなく「**自然な写真に近づけるプロンプトを組み立てる**」と事実ベースで（第1弾の学び＝動作事実で言い切る）。
- **実装難所**: エンジニアリングは軽い（フォーム＋テンプレートエンジン＋コピー機能）。本質は**質問フロー設計＋AI臭さ消しのプロンプト構築知識ベース**＝Web記事収集→Mado構造化→Sara文面整形が主役（Cody実装より上流が肝）。工数の重心は実装より知識ベース＝総計2.5〜4週見立て。

#### F2前半 確定事項（黒田さん承認 2026-06-19）

| 論点 | 確定 |
|---|---|
| slug | **`ai-image-prompt` で確定**（表示名「AI画像プロンプト・ビルダー」）。以後不変＝301回避。ドラフトフォルダは `docs/ai-image-prompt/` に確定済み |
| 公開スコープ | **完成形で出す**（ミニマルにしない） |
| 知識ベース（AI臭さ消しcraft）の体制 | craft の種は黒田さん発ではなく **Web記事収集で原料化**（黒田さん本人にAI画像ノウハウがないため）→ **Mado構造化 → Sara文面整形**。重視軸＝「Web制作で使う画像＝実際にカメラで撮った自然な写真に近づける」。一次ソース集は `_research-knowledge-base.md`（出典付き・主要手法3ソース以上クロスチェック済）で作成済み |
| ★2 機種名/フィルム銘柄 | **任意トグル**（必須コアに入れない） |
| ★3 ネガティブ重み記法（`:1.3`） | **汎用版は重み無し**（モデル固有記法は注記レベルに隔離） |
| 実機検証の方針 | 机上仕様を一気に書かず、**光1カテゴリを縦スライスで黒田さんが実機検証→効く型を確定→質感/構図/人物へ横展開**（第1弾「机上→実機書き換え」教訓の再発防止）。検証キット `_f2-lighting-validation-kit.md`（2題×光4段階L0〜L3×日英＝16プロンプト＋判定軸C1〜C6＋結果記入欄）作成済み |
| Mane評価ゲート | overview は **条件付き合格**（実現性・スコープ・マネタイズ整合OK／工数の重心は実装より知識ベース＝総計2.5〜4週見立て）。評価レポート＝`docs/ai-image-prompt/_f2-mane-review.md` |

#### F2後半 確定事項（実機検証〜spec完成 / 2026-06-24〜07-01・F2完全クローズ）

| 論点 | 確定 |
|---|---|
| 光プリセット（実機確定 Google Flow/Imagen 4） | **必須コア＝方向＋落ちる影**／加える＝柔らかい影・拡散光／**時間帯は任意・既定オフ**（単独は逆効果）。※机上リサーチ（3点セット）と実機がズレ、実機で補正。検証キット＝`_f2-lighting-validation-kit.md` §6-C |
| 質感プリセット（実機確定） | **必須コア＝自然なノイズ/film grain**（物・素材で効く）／素材ディテール等は任意上乗せ／**人物の肌質感は控えめ**（高品質モデルでは素で十分）。検証キット＝`_f2-texture-validation-kit.md` §6-C |
| 設計の軸＝被写体タイプ分岐 | **人物＝"光"が主役／物・空間＝"質感"が主役**（光検証と真逆の対称性）。この非対称性を spec の分岐に反映 |
| spec本体完成（`01_spec.md`） | 9カテゴリ質問フロー・**二層構造（コア5問＋深掘り4段）**・カテゴリ別v1本数の天井（**合計約60 optionKey**）・決定論ロジック・日英対辞書・**ネガティブ別欄（重み無し）**・参考AI注記（年付き）・**★2機種名任意トグル**。**人物肌質感は⑦person_poresに一本化**（⑤質感は物・素材専念・de-dup不要化） |
| spec評価ゲート（Mane） | **条件付き合格＝F3着手可**。F3実装（Cody）工数＝**S〜M（実働3〜6日）**・第1弾より軽い（WASM/Worker無し）。評価レポート＝`_f2-spec-mane-review.md` |
| SEO/QA完成（`03_seo.md`/`02_qa.md`） | プライマリKW＝`ai 画像 プロンプト 作り方`／**WebApplication＋BreadcrumbList**／過剰約束なしコピー／受け渡し型・日英・決定論・kiso.css地雷を合格条件付きでQA |

- **F3着手前の軽微な残論点（Sara★・F3で確定）**: ①構造化データ `applicationCategory`＝第1弾`MultimediaApplication`に揃えるか`DesignApplication`か（F3で確定）②パンくずラベルは第1弾も和文化済みで和文統一＝食い違いなし（共通partial化はF3で）。
- **F2の未決（実機検証待ちブロック）は全解消**＝光/質感プリセットが実機で確定し、spec本体（`01_spec.md`）に横展開済み。**次の起点＝F3実装（Cody）**：static-template基盤で起こす（観点UU）／`01_spec.md` を設計図に。
- **F2成果物（`docs/ai-image-prompt/`）**: `00_overview.md`（9カテゴリ）／`01_spec.md`（spec本体）／`02_qa.md`／`03_seo.md`／`_f2-mane-review.md`（overview評価）／`_f2-spec-mane-review.md`（spec評価）／`_research-knowledge-base.md`（一次ソース集）／`_f2-lighting-validation-kit.md`（光検証キット）／`_f2-texture-validation-kit.md`（質感検証キット）。
- **参照**: markloom（`https://markloom.satoapps.com/`）＝受け渡し型ツールの先行例（ブラウザ完結・AI実行はユーザー側・無料・キー不要）。
...
| ~~U-04~~ | 第2弾ツールの決定 | **✅決定済み（2026-06-19）＝候補カタログ外の黒田さん発案ツール「AI画像プロンプト・ビルダー」（slug=`ai-image-prompt`）に確定**。決定論的ビルダー単体／日英切替／モデル非依存／受け渡し型（markloom型）。候補スコアリング（Mado）はスキップ。**F2前半確定済み**（slug・スコープ完成形・知識ベース体制・★2/★3・実機検証方針・Mane条件付き合格）。詳細＝§2-10。次＝光カテゴリ実機検証（黒田さん宿題） | ~~第1弾公開後~~ **決定済み** | 黒田さん（候補外発案） |
...
| F4 第1弾 QA・評価 | ✅ 完了（2026-06-18・コードレベル） | Reid最終評価ゲート＝条件付き合格→Must M-2（wasm-feature-detect明示）/S-1（準備中デッドコード除去）/S-3（worker再生成）反映。後発UI変更（スイッチUI・タイトル・区切り線・並べ替え）も再チェック合格。評価レポート＝`docs/image-compress/_f4-reid-final-review.md`/`_f4b-reid-recheck.md`。**実機QA（黒田さん）＝完了OK（2026-06-18）**＝30枚AVIF固まり実測・Safari降格・実画像で品質/④減色256調整・EXIF正立・リッチリザルトテストをすべて確認OK。残=なし（次はF6デプロイ） |
...
| F6 デプロイ・公開 | ✅ 完了（公開日 2026-06-19） | **公開URL＝`kurodafolio.com/tools/`（ポータル）／`kurodafolio.com/tools/image-compress/`（ツール）**。黒田さんが FileZilla/FTP で `dev/dist/` を `public_html/tools/` へ・本体 `dist/` を `public_html/` へアップロードし実機確認済み（ランブック A〜E チェック済み＝`docs/_portal/04_f6-deploy-runbook.md`）。完了基準充足＝①3URL到達（`/tools/`・`/tools/image-compress/`・末尾スラッシュ無し301）②ツール実機動作（WASM読込/WebP・AVIF生成/個別・一括ZIP DL）③`.wasm`=`application/wasm`（B-3）④本体`/`との干渉なし（本体`.htaccess`に全捕捉rewrite無し・`tools/`温存／**公開後に発生した kiso.css 404・MIME問題は修正・解消済み＝公開後対応として完了**）⑤本体導線①②③＋逆導線（C 実機OK）⑥OGP3対象を公開URL表示確認＋X/Facebook キャッシュ再取得（D）⑦サイトマップ再送信（E・成功）。**任意残**＝Search Console URL検査でのインデックス登録リクエスト2URL（サイトマップ再送信済みで自然に拾われるため任意）。詳細はランブック参照。Mane 公開チェック＋代表（黒田さん）確認済み |
| **【第2弾】F2 AI画像プロンプト・ビルダー 設計** | ✅ 完了（2026-07-01・機能設計 完全クローズ） | slug=`ai-image-prompt` 確定・公開スコープ＝完成形・知識ベース体制確定。**光/質感プリセットを実機確定（Google Flow/Imagen 4）→被写体タイプ分岐（人物=光/物・空間=質感）を軸に spec本体 `01_spec.md` 完成（9カテゴリ・二層構造・約60 optionKey・決定論・日英対辞書・ネガティブ別欄・人物肌質感⑦一本化）→spec評価ゲート条件付き合格＝F3着手可（Mane）→SEO/QA完成**。詳細＝§2-10「F2前半／F2後半 確定事項」。**次の起点＝F3実装（Cody・static-template基盤・工数S〜M）。F2ステップ別の消化は下記サブテーブル参照** |

#### 第2弾 F2 ステップ別 進捗（2026-07-01 Mane・全ステップ完了＝F2クローズ）

| F2ステップ | 状態 | 成果物／備考 |
|---|---|---|
| overview（slug確定・9カテゴリ） | ✅ 完了 | `docs/ai-image-prompt/00_overview.md`（slug=`ai-image-prompt`・9カテゴリ） |
| Mane評価ゲート（overview） | ✅ 完了（条件付き合格） | overview の実現性・スコープ・マネタイズ整合OK／工数の重心は実装より知識ベース。`_f2-mane-review.md` |
| craft知識ベース リサーチ | ✅ 完了 | `_research-knowledge-base.md`（出典付き・主要手法3ソース以上クロスチェック済）。重視軸＝Web制作画像を「実際にカメラで撮った自然な写真」に近づける |
| 光カテゴリ実機検証 | ✅ 完了 | `_f2-lighting-validation-kit.md` §6-C。実機（Google Flow/Imagen 4）で確定＝**必須コア＝方向＋落ちる影**／柔らかい影・拡散光は加える／時間帯は任意・既定オフ。※机上リサーチと実機がズレ実機で補正 |
| 質感カテゴリ実機検証 | ✅ 完了 | `_f2-texture-validation-kit.md` §6-C。**必須コア＝自然なノイズ/film grain**（物・素材で効く）／素材ディテールは任意上乗せ／人物の肌質感は控えめ。**被写体タイプ分岐（人物=光/物・空間=質感）を確定** |
| spec本体（`01_spec.md`） | ✅ 完了 | 9カテゴリ質問フロー・二層構造（コア5問＋深掘り4段）・カテゴリ別v1天井≈60 optionKey・決定論ロジック・日英対辞書・ネガティブ別欄（重み無し）・参考AI注記（年付き）・★2機種名任意トグル・人物肌質感は⑦person_poresに一本化 |
| spec評価ゲート（Mane） | ✅ 完了（条件付き合格＝F3着手可） | `_f2-spec-mane-review.md`。F3実装（Cody）工数＝S〜M（実働3〜6日）・第1弾より軽い（WASM/Worker無し） |
| seo・qa（`03_seo.md`/`02_qa.md`） | ✅ 完了 | プライマリKW=`ai 画像 プロンプト 作り方`／WebApplication＋BreadcrumbList／過剰約束なしコピー／受け渡し型・日英・決定論・kiso.css地雷を合格条件付きでQA。★Sara残論点（applicationCategory・パンくずpartial化）はF3で確定 |

> **F2完全クローズ（2026-07-01）**。次の起点＝**F3実装（Cody）**：static-template基盤で起こす（観点UU）／`01_spec.md` を設計図に。フェーズ完了ごとに本テーブルを更新する。
...
## 8-3. 公開後の申し送り（第1弾クローズ後 / 2026-06-19 Mane）

F6 完了＝第1弾「画像仕分け圧縮くん」を `kurodafolio.com/tools/` に本番公開しクローズ。以後の打ち手を整理する（着手順は黒田さん判断）。

| # | 申し送り | 内容 | 優先 |
|---|---|---|---|
| P-1 | 記事⇄ツールの文脈リンクで3点回遊を強化 | frontend-note の関連記事（画像最適化/WebP/AVIF 等）⇄ツールを**文脈リンク**で相互送客し、本体トップ・ツール・記事の3点回遊を強める（§2-9 マネタイズ方針＝記事側AdSenseが収益器・ツール本体は広告フリーのまま）。**フッターでの宣伝強化は不採用**（黒田さん判断済み） | 推奨 |
| ~~P-2~~ | 第2弾ツールの決定（U-04） | **✅決着（2026-06-19）＝AI画像プロンプト・ビルダー（slug=`ai-image-prompt`・候補カタログ外の黒田さん発案）に確定**。決定論的ビルダー単体／日英切替／モデル非依存／受け渡し型（markloom型）。**F2（機能設計）完全クローズ（2026-07-01）**＝光/質感プリセット実機確定・被写体タイプ分岐・spec本体`01_spec.md`完成・spec評価ゲート条件付き合格＝F3着手可・SEO/QA完成（詳細＝§2-10 / §8 進捗表）。次＝**F3実装（Cody・static-template基盤・工数S〜M）**。OGP・共通枠・デプロイ手順は第1弾で確立済み（再利用OGPテンプレ＝`docs/_portal/design/ogp-template/`）＝量産しやすい | 次＝F3実装 |
| P-3 | インデックス登録リクエスト（任意残） | ランブック §E の URL検査ツールでの2URL（`/tools/`・`/tools/image-compress/`）登録リクエストは**任意残**。サイトマップ再送信済みで自然にクロール・インデックスされるため必須ではない | 任意 |
| P-4 | 公開後モニタリング（任意） | Search Console の検索パフォーマンス（"画像圧縮 webp" 等の流入）／実利用フィードバックを折を見て確認。ファネル（ツール流入→人物認知→案件相談）の傍証になり得る | 任意 |

> 詳細手順・チェック結果＝`docs/_portal/04_f6-deploy-runbook.md`。マネタイズ・送客方針の正本＝§2-9。
