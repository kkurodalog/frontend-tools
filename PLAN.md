...
| P-2 | 第2弾ツールの優先順決定（U-04） | clamp / color-palette / flocss-generator / motion の着手順を黒田さん＋Mado で決定（候補カタログ＝`docs/_portal/mado-candidates.md`・温存 motion 仕様＝`docs/motion/`）。OGP・共通枠・デプロイ手順は第1弾で確立済み（再利用OGPテンプレ＝`docs/_portal/design/ogp-template/`）＝2弾目以降は量産しやすい | 次の主タスク |
