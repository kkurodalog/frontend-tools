...
- **F2で残る未決（次の起点）**: **光カテゴリの実機検証結果待ち**（黒田さん宿題＝検証キット消化）。検証で「効く型」が確定するまで spec 本体（光プリセット→質感/構図/人物へ横展開）／seo・qa はブロック中。
- **F2成果物（`docs/ai-image-prompt/`）**: `00_overview.md`（9カテゴリ）／`_f2-mane-review.md`（Mane評価）／`_research-knowledge-base.md`（一次ソース集）／`_f2-lighting-validation-kit.md`（光検証キット）。
...
| **【第2弾】F2 AI画像プロンプト・ビルダー 設計** | 🔄 前半完了・**実機検証待ちでブロック中**（2026-06-19） | slug=`ai-image-prompt` 確定（表示名「AI画像プロンプト・ビルダー」）・公開スコープ＝完成形・知識ベース体制（Web記事収集→Mado構造化→Sara文面整形）確定。詳細＝§2-10「F2前半 確定事項」。**F2ステップ別の現在地（次の起点＝光カテゴリ実機検証）は下記サブテーブル参照** |

#### 第2弾 F2 ステップ別 進捗（2026-06-19 Mane）

| F2ステップ | 状態 | 成果物／備考 |
|---|---|---|
| overview（slug確定・9カテゴリ） | ✅ 完了 | `docs/ai-image-prompt/00_overview.md`（slug=`ai-image-prompt`・9カテゴリ） |
| Mane評価ゲート | ✅ 完了（条件付き合格） | overview の実現性・スコープ・マネタイズ整合OK／工数の重心は実装より知識ベース＝総計2.5〜4週見立て。`_f2-mane-review.md` |
| craft知識ベース リサーチ | ✅ 完了 | `_research-knowledge-base.md`（出典付き・主要手法3ソース以上クロスチェック済）。重視軸＝Web制作画像を「実際にカメラで撮った自然な写真」に近づける |
| 光カテゴリ実機検証キット | ✅ 作成済 → **検証は黒田さん宿題（未着手＝次の起点）** | `_f2-lighting-validation-kit.md`（2題×光4段階L0〜L3×日英＝16プロンプト＋判定軸C1〜C6＋結果記入欄）。黒田さんが実機検証して「効く型」を確定するのが次フェーズの起点 |
| spec本体（光プリセット確定→質感/構図/人物へ横展開）／seo・qa | ⛔ 未着手（実機検証待ちでブロック中） | 光カテゴリの縦スライス検証で効く型が確定してから着手（机上で一気に書かない＝第1弾「机上→実機書き換え」教訓の再発防止） |
...
| ~~P-2~~ | 第2弾ツールの決定（U-04） | **✅決着（2026-06-19）＝AI画像プロンプト・ビルダー（slug=`ai-image-prompt`・候補カタログ外の黒田さん発案）に確定**。決定論的ビルダー単体／日英切替／モデル非依存／受け渡し型（markloom型）。**F2前半完了（2026-06-19）**＝slug・スコープ完成形・知識ベース体制・★2/★3・実機検証方針・Mane条件付き合格（詳細＝§2-10 / §8 進捗表）。次＝**光カテゴリ実機検証（黒田さん宿題）→効く型確定→spec横展開**。OGP・共通枠・デプロイ手順は第1弾で確立済み（再利用OGPテンプレ＝`docs/_portal/design/ogp-template/`）＝量産しやすい | F2進行中（実機検証待ち） |
